...
func (e *EventProcessor) Run(ctx context.Context) {
	...
	if err := e.grpcServer.Run(); err != nil {
		log.Logger.Error("gRPC stopped with error", zap.Error(err))
	}
}
...
func (e *EventProcessor) ProcessEvents(ctx context.Context) {
	...
	log.Logger.Debug("start event processing goroutines")
...
func (e *EventProcessor) UpdateOwnerAndProvenance(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			log.Logger.Error("Have error when try to get queue event", zap.Error(err))
		}

		if event == nil {
			log.Logger.Info("event queue empty")
		...
		if token != nil {
			...
			if len(accounts) == 0 {
				...
				if err := e.indexerStore.IndexAccountTokens(ctx, to, []indexer.AccountToken{accountToken}); err != nil {
					log.Logger.Error("cannot index a new account_token", zap.String("indexID", indexID), zap.String("owner", to))
				}
			}
		...
		if err := e.UpdateEvent(eventID, map[string]interface{}{
			...
			log.Logger.Error("fail to update event", zap.Error(err))
		}
...
func (e *EventProcessor) logStartStage(event *NFTEvent, stage int8) {
	log.Logger.Sugar().Infof("start stage %d for event: ", stage, zap.Any("event", event))
}

// logEndStage log when end a stage
func (e *EventProcessor) logEndStage(event *NFTEvent, stage int8) {
	log.Logger.Sugar().Infof("finished stage %d for event: ", stage, zap.Any("event", event))
}
...
func (e *EventProcessor) UpdateLatestOwner(ctx context.Context) {
	...
	for {
		...
		token, err := e.indexerStore.GetTokensByIndexID(ctx, indexID)
		if err != nil {
			log.Logger.Error("fail to get token by index id", zap.Error(err))
		}
		...
		if !token.Fungible {
			...
			if err != nil {
				...
				if err != nil {
					log.Logger.Error("fail to update owner", zap.Error(err))
				}
			}
		} else {
			err := e.indexerStore.UpdateOwnerForFungibleToken(ctx, indexID, token.LastRefreshedTime, event.To, 1)
			if err != nil {
				log.Logger.Error("fail to update owner for fungible token", zap.Error(err))
			}
		}
		...
		if err := e.indexerStore.IndexAccountTokens(ctx, to, accountTokens); err != nil {
			log.Logger.Error("fail to index account token", zap.Error(err))
			continue
		}
	...
	for {
		...
		for _, accountID := range accounts {
			if err := e.notifyChangeOwner(accountID, to, indexID); err != nil {
				log.Logger.Error("fail to send notificationSdk for the new update",
					zap.Error(err),
...
func (e *EventProcessor) SendEventToFeedServer() {
	...
	for {
		...
		if err := e.queueProcessor.store.CompleteEvent(eventID); err != nil {
			log.Logger.Error("fail to mark an event completed", zap.Error(err))
		}
